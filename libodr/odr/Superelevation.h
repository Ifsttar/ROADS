...
//! \file Superelevation.h
//! \brief odr::Superelevation class declaration

#ifndef _ODR_SUPERELEVATION_H_
#define _ODR_SUPERELEVATION_H_
...
namespace odr
{
  ...
  //! Store superelevation information for a section of a road.
  //! Wraps one OpenDRIVE superelevation record.
  ...
  public:
    ...
    { if(p) static_cast<Superelevation*>(p)->destroy(); }

    //! Return a pointer to parent lateral profile.
    virtual LateralProfile* ODR_CALL parentProfile( void ) = 0;

    //! Return a pointer to parent lateral profile, read-only.
    virtual const LateralProfile* ODR_CALL parentProfile( void ) const = 0;

    //! @name Read-only accessors
    //@{

    //! Return start position (s-coord) of this record relative to road.
    virtual double ODR_CALL start( void ) const = 0;

    //! Return the length along the S-coordinate for which the superelevation
    ...
    //! Length of every superelevation record in road is re-computed
    //! accordingly.
...
#endif  // #ifndef _ODR_SUPERELEVATION_H_
